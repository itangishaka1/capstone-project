...
exports.index = (req, res) => {
  knex('course')
    .select('id', 'admin_id','course_name', 'course_type','course_department','semester','credit',)
    .then((data) => {
      const courses = data.filter(course => course.admin_id === req.decoded.id)
      res.status(200).json(courses);
    })
    .catch((err) => res.status(400).send(`Error retrieving course ${err}`));
};
